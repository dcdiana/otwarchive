class CommentMailer < ActionMailer::Base
  ...
  def comment_notification(user, comment)
    ...
    I18n.with_locale(Locale.find(user.preference.preferred_locale).iso) do
      mail(
        to: user.email,
        subject: "[#{Configurable.APP_SHORT_NAME}] Comment on " + (@comment.ultimate_parent.is_a?(Tag) ? "the tag " : "") + @comment.ultimate_parent.commentable_name.gsub("&gt;", ">").gsub("&lt;", "<")
      )
    end
    ensure
  ...
  def edited_comment_notification(user, comment)
    ...
    I18n.with_locale(Locale.find(user.preference.preferred_locale).iso) do
      mail(
        to: user.email,
        subject: "[#{Configurable.APP_SHORT_NAME}] Edited comment on " + (@comment.ultimate_parent.is_a?(Tag) ? "the tag " : "") + @comment.ultimate_parent.commentable_name.gsub("&gt;", ">").gsub("&lt;", "<")
      )
    end
    ensure
      I18n.locale = I18n.default_locale
  ...
  def comment_reply_notification(your_comment, comment)
    ...
    mail(
      ...
      subject: "[#{Configurable.APP_SHORT_NAME}] Reply to your comment on " + (@comment.ultimate_parent.is_a?(Tag) ? "the tag " : "") + @comment.ultimate_parent.commentable_name.gsub("&gt;", ">").gsub("&lt;", "<")
    )
    ensure
      I18n.locale = I18n.default_locale
  end
  ...
  def edited_comment_reply_notification(your_comment, edited_comment)
    ...
    mail(
      to: @your_comment.comment_owner_email,
      subject: "[#{Configurable.APP_SHORT_NAME}] Edited reply to your comment on " + (@comment.ultimate_parent.is_a?(Tag) ? "the tag " : "") + @comment.ultimate_parent.commentable_name.gsub("&gt;", ">").gsub("&lt;", "<")
    )
    ensure
      I18n.locale = I18n.default_locale
  end
  ...
  def comment_sent_notification(comment)
    ...
    mail(
      ...
      subject: "[#{Configurable.APP_SHORT_NAME}] Comment you left on " + (@comment.ultimate_parent.is_a?(Tag) ? "the tag " : "") + @comment.ultimate_parent.commentable_name.gsub("&gt;", ">").gsub("&lt;", "<")
    )
    ensure
      I18n.locale = I18n.default_locale
  end
