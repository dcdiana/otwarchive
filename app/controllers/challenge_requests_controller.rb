class ChallengeRequestsController < ApplicationController
  ...
  def index
    ...
    if @sort_column == "fandom"
      ...
      @requests = Prompt.paginate_by_sql(query, :page => params[:page], :per_page => Configurable.ITEMS_PER_PAGE)
      ...
      @requests = @collection.prompts.where("type = 'Request'").order(@sort_order).paginate(:page => params[:page], :per_page => Configurable.ITEMS_PER_PAGE)
    end
  end
