class PseudsController < ApplicationController
  ...
  def show
    ...
    @works = visible_works.revealed.non_anon.order("revised_at DESC").limit(Configurable.NUMBER_OF_ITEMS_VISIBLE_IN_DASHBOARD)
    @series = visible_series.order("updated_at DESC").limit(Configurable.NUMBER_OF_ITEMS_VISIBLE_IN_DASHBOARD)
    @bookmarks = visible_bookmarks.order("updated_at DESC").limit(Configurable.NUMBER_OF_ITEMS_VISIBLE_IN_DASHBOARD)
