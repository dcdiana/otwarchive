class FavoriteTag < ActiveRecord::Base
  ...
  def within_limit
    if user.favorite_tags(:reload).count >= Configurable.MAX_FAVORITE_TAGS
      errors.add(:base, ts('Sorry, you can only save %{maximum} favorite tags.', maximum: Configurable.MAX_FAVORITE_TAGS))
    end
  end
